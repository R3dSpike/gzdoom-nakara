...
// [NKS] Reset warmup completion latch on every new map without clearing
// prepared buffers that a loading-screen map may have built for this map.
extern void NKWarmup_OnLevelStart(void);
...
void P_PrecacheActorClass(PClassActor* cls)
{
	if (demoplayback)
	{
		return;
	...
	static TMap<PClassActor*, bool> manualPrecachedActors;

	if (manualPrecachedActors.CheckKey(cls))
	{
		return;
	}

	manualPrecachedActors[cls] = true;
