...
EXTERN_CVAR(Bool,in_gamepadconnected)
void DrawHUD();
void D_DoAnonStats();
void I_DetectOS();
...
static int D_DoomMain_Internal (void)
{
	...
	do
	{
		...
		if (ret == 0)
		{
			// [NKS] Load the decoded texture disk-cache index during startup instead of
			// letting the first sprite cache hit/miss do directory cleanup in gameplay.
			NKDiskCache_Init();
		}
		pwads.clear();
		pwads.shrink_to_fit();
