...
//===========================================================================
//
// [Nakara] Build the same object-to-world transform used by RenderModel, but
// for an arbitrary trail sample position. The compact cached model hull is
// transformed through this matrix and later projected to a path-orthogonal
// silhouette by the hardware sprite renderer.
//
//===========================================================================

static VSMatrix BuildParticleTrailModelMatrix(const DVector3& worldPos, FSpriteModelFrame* smf, AActor* actor, double ticFrac, const DRotator* anglesOverride)
{
	int smf_flags = smf->getFlags(actor->modelData);
	...
	double rotateOffset = 0;
	DRotator angles;
	if (anglesOverride != nullptr)
		angles = *anglesOverride;
	else if (actor->renderflags & RF_INTERPOLATEANGLES)
		angles = actor->InterpolatedAngles(ticFrac);
	else
		angles = actor->Angles;
	float angle = angles.Yaw.Degrees();
	...
	if (smf_flags & MDL_PITCHFROMMOMENTUM)
	{
		const double vx = actor->Vel.X;
		const double vy = actor->Vel.Y;
		const double vz = actor->Vel.Z;
		if (actor->Vel.LengthSquared() > EQUAL_EPSILON)
		{
			if (vz || vx || vy) pitch = float(atan(vz / sqrt(vx * vx + vy * vy)) / M_PI * 180);
			if (fabs(vx) > EQUAL_EPSILON || fabs(vy) > EQUAL_EPSILON)
			{
				if ((vx * cos(angle * M_PI / 180) + vy * sin(angle * M_PI / 180)) / sqrt(vx * vx + vy * vy) < 0) pitch *= -1;
			}
			else pitch = fabs(pitch);
		}
	}
	...
	VSMatrix matrix;
	matrix.loadIdentity();
	matrix.translate((float)worldPos.X, (float)worldPos.Z, (float)worldPos.Y);
	...
	{
		stretch = (smf->modelIDs[0] >= 0 ? Models[smf->modelIDs[0]]->getAspectFactor(actor->Level->info->pixelstretch) : 1.f) / actor->Level->info->pixelstretch;
		matrix.scale(1, stretch, 1);
	}

	if (smf_flags & MDL_USEROTATIONCENTER)
	{
		matrix.translate(smf->rotationCenterX, smf->rotationCenterZ / stretch, smf->rotationCenterY);
	}
	matrix.rotate(-angle, 0, 1, 0);
	matrix.rotate(pitch, 0, 0, 1);
	matrix.rotate(-roll, 1, 0, 0);
	if (smf_flags & MDL_USEROTATIONCENTER)
	{
		matrix.translate(-smf->rotationCenterX, -smf->rotationCenterZ / stretch, -smf->rotationCenterY);
	}
	...
	if (smf_flags & MDL_ROTATING)
	{
		matrix.translate(smf->rotationCenterX, smf->rotationCenterY / stretch, smf->rotationCenterZ);
		matrix.rotate((float)rotateOffset, smf->xrotate, smf->yrotate, smf->zrotate);
		matrix.translate(-smf->rotationCenterX, -smf->rotationCenterY / stretch, -smf->rotationCenterZ);
	}

	matrix.scale(scaleFactorX, scaleFactorZ, scaleFactorY);
	matrix.translate(smf->xoffset / smf->xscale, smf->zoffset / (smf->zscale * stretch), smf->yoffset / smf->yscale);
	matrix.rotate(-smf->angleoffset, 0, 1, 0);
	matrix.rotate(smf->pitchoffset, 0, 0, 1);
	matrix.rotate(-smf->rolloffset, 1, 0, 0);
...
bool GetParticleTrailModelWorldPoints(AActor* actor, FSpriteModelFrame* smf, const DVector3& worldPos, double ticFrac, const DRotator* anglesOverride, TArray<FVector3>& outPoints)
{
	outPoints.Clear();
	if (actor == nullptr || actor->Level == nullptr || smf == nullptr) return false;

	DActorModelData* modelData = actor->modelData.ForceGet();
	...
	CalcModelFrameInfo frameinfo = CalcModelFrame(actor->Level, smf, actor->state, actor->tics, modelData, actor, is_decoupled, tic);
	ModelDrawInfo drawinfo;
	VSMatrix matrix = BuildParticleTrailModelMatrix(worldPos, smf, actor, ticFrac, anglesOverride);

	for (unsigned i = 0; i < frameinfo.modelsamount; ++i)
	{
		if (!CalcModelOverrides(i, smf, modelData, frameinfo, drawinfo, is_decoupled)) continue;
		if (drawinfo.modelid < 0 || drawinfo.modelid >= (int)Models.Size()) continue;

		const auto& hull = Models[drawinfo.modelid]->GetParticleTrailHull();
		for (unsigned h = 0; h < hull.Size(); ++h)
		{
			FLOATTYPE in[4] = { (FLOATTYPE)hull[h].X, (FLOATTYPE)hull[h].Y, (FLOATTYPE)hull[h].Z, 1 };
			FLOATTYPE res[4];
			matrix.multMatrixPoint(in, res);
			// VSMatrix uses render coordinates (X, Z, Y). Convert back to engine XYZ.
			outPoints.Push(FVector3((float)res[0], (float)res[2], (float)res[1]));
		}
	}
	return outPoints.Size() >= 3;
}

bool GetParticleTrailModelWorldPoints(AActor* actor, FSpriteModelFrame* smf, const DVector3& worldPos, double ticFrac, TArray<FVector3>& outPoints)
{
	return GetParticleTrailModelWorldPoints(actor, smf, worldPos, ticFrac, nullptr, outPoints);
}
