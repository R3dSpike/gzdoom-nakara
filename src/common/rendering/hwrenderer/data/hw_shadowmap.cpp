...
int IShadowMap::NkSpriteProjectors;
...
{
	FString out;
	out.Format("upload=%04.2f ms  lights=%d  shadowmapped=%d  nkProjectors=%d", IShadowMap::UpdateCycles.TimeMS(), IShadowMap::LightsProcessed, IShadowMap::LightsShadowmapped, IShadowMap::NkSpriteProjectors);
	return out;
}
...
bool IShadowMap::PerformUpdate()
{
	...
	if (CollectLights != nullptr)
	{
		...
		UploadAABBTree();
		UploadLights();
		UploadNkSpriteShadowProjectors();
		return true;
	}
	return false;
}
...
void IShadowMap::DisableNkSpriteShadowProjectors()
{
	ClearNkSpriteShadowProjectors();
	UploadNkSpriteShadowProjectors();
}

void IShadowMap::UploadNkSpriteShadowProjectors()
{
	// Keep one dummy projector in the SSBO so material shaders with an unsized
	// storage buffer never see an unbound/zero-sized buffer. The shader treats a
	// projector with strength <= 0 as inactive.
	NkSpriteProjectors = (int)mNkSpriteShadowProjectors.Size();
	if (mNkSpriteShadowProjectors.Size() == 0)
	{
		NkSpriteShadowProjector dummy = {};
		mNkSpriteShadowProjectors.Push(dummy);
	}

	if (mNkSpriteShadowProjectorsBuffer == nullptr)
		mNkSpriteShadowProjectorsBuffer = screen->CreateDataBuffer(NKSPRITEPROJECTORS_BINDINGPOINT, true, false);

	mNkSpriteShadowProjectorsBuffer->SetData(sizeof(NkSpriteShadowProjector) * mNkSpriteShadowProjectors.Size(), &mNkSpriteShadowProjectors[0], BufferUsageType::Stream);
}
...
void IShadowMap::Reset()
{
	...
	delete mNkSpriteShadowProjectorsBuffer; mNkSpriteShadowProjectorsBuffer = nullptr;
