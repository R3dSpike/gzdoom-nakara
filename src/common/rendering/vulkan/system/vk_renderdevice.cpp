...
//===========================================================================
//
// NKS background cache stage 2 for Vulkan.
// This mirrors the conservative OpenGL queue/pacing path: warmup jobs can be
// queued and then drained through Vulkan's normal main-thread PrecacheMaterial
// path. It does not enable true async Vulkan transfer uploads yet. The goal is
// to avoid letting one warmup burst immediately upload many textures in a row.
//
//===========================================================================
...
bool VulkanRenderDevice::SupportsBackgroundCache()
{
...
bool VulkanRenderDevice::NKBGQueueMaterial(FMaterial *mat, int translation, bool makeSPI, bool secondary)
{
	...
	{
		Printf("[NKBGCache/VK] queued mat=%p translation=%d queue=%u upload_thread=%d pacing=%d\n",
...
int VulkanRenderDevice::NKBGEffectiveLimit(bool flush) const
{
	if (flush) return 0;
...
int VulkanRenderDevice::NKBGEffectiveBudgetMS(bool flush) const
{
...
int VulkanRenderDevice::NKBGEffectiveSlowMS() const
{
...
int VulkanRenderDevice::NKBGEffectiveCooldownFrames() const
{
...
void VulkanRenderDevice::UpdateBackgroundCache(bool flush)
{
	...
	// Stage 5A: conservative render-thread Vulkan upload pacing. This still uses
	// the normal PrecacheMaterial/GetImage path; it does not enable an async
	// Vulkan transfer queue yet.
	...
	{
		NKVKQueuedMaterial job = NKBGCachedMaterials[0];
	...
	{
		Printf("[NKGPUUpload/VK] processed=%d remaining=%u submitted=%u consumed=%u flush=%d loading=%d limit=%d budget=%dms batch=%.3fms max=%.3fms slow=%u budget_hit=%d cooldown=%d\n",
			processed,
...
void VulkanRenderDevice::PrintBackgroundCacheStatus()
{
	bool active = NKBGCachedMaterials.Size() > 0 || NKBGThrottleFramesLeft > 0 || NKBGLastProcessed > 0;
	Printf("[NKGPUUpload/VK] enabled=%d pacing=%d loading=%d active=%d queue=%u submitted=%u consumed=%u collisions=%u upload_thread=%d limit=%d budget=%dms slow_ms=%d cooldown=%d/%d upload_frames=%u batches=%u slow_jobs=%u slow_batches=%u budget_hits=%u def_budget=%u def_cooldown=%u flushes=%u total=%.3fms peak_job=%.3fms peak_batch=%.3fms peak_batch_jobs=%d peak_at=%u/%u last_jobs=%d last_batch=%.3fms last_job=%.3fms last_budget_hit=%d\n",
...
void VulkanRenderDevice::ResetBackgroundCacheStats()
{
...
void VulkanRenderDevice::StopBackgroundCache()
{
	...
	{
		Printf("[NKBGCache/VK] stop cleared remaining=%u submitted=%u consumed=%u collisions=%u\n",
