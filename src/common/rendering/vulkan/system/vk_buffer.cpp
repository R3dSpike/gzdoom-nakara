...
void VkBufferManager::RemoveBuffer(VkHardwareBuffer* buffer)
{
	...
	for (VkHardwareDataBuffer** knownbuf : { &ViewpointUBO, &LightBufferSSO, &LightNodes, &LightLines, &LightList, &BoneBufferSSO, &NkSpriteProjectorsSSO })
	{
...
IDataBuffer* VkBufferManager::CreateDataBuffer(int bindingpoint, bool ssbo, bool needsresize)
{
	...
	case NKSPRITEPROJECTORS_BINDINGPOINT: NkSpriteProjectorsSSO = buffer; break;
