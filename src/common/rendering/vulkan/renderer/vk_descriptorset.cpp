...
static VkHardwareDataBuffer* NKEnsureVkSpriteProjectorBuffer(VulkanRenderDevice* fb)
{
	// [Nakara] Receiver sprite shadows use a material SSBO at set=1 binding=5.
	// The buffer normally gets filled by IShadowMap::UploadNkSpriteShadowProjectors(),
	// but Vulkan descriptor sets can be updated before the first shadow update. Create
	// a single inactive dummy projector here so the descriptor is always valid.
	if (screen->mShadowMap.mNkSpriteShadowProjectorsBuffer == nullptr)
	{
		NkSpriteShadowProjector dummy = {};
		screen->mShadowMap.mNkSpriteShadowProjectorsBuffer = screen->CreateDataBuffer(NKSPRITEPROJECTORS_BINDINGPOINT, true, false);
		screen->mShadowMap.mNkSpriteShadowProjectorsBuffer->SetData(sizeof(dummy), &dummy, BufferUsageType::Static);
	}
	return static_cast<VkHardwareDataBuffer*>(screen->mShadowMap.mNkSpriteShadowProjectorsBuffer);
}
...
void VkDescriptorSetManager::UpdateHWBufferSet()
{
	...
	auto nkSpriteProjectorBuffer = NKEnsureVkSpriteProjectorBuffer(fb);
	...
	WriteDescriptors()
		...
		.AddBuffer(HWBufferSet.get(), 5, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, nkSpriteProjectorBuffer->mBuffer.get())
