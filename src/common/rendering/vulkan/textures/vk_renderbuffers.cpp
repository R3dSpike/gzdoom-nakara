...
VkSampleCountFlagBits VkRenderBuffers::GetBestSampleCount()
{
	...
	// Nakara focus and cloak auxiliary textures are sampled by regular sampler2D
	// postprocess bindings. Force the scene to single-sample while either feature
	// is active so Vulkan never exposes unresolved MSAA attachments to those passes.
...
void VkRenderBuffers::CreateScene(int width, int height, VkSampleCountFlagBits samples)
{
	...
	CreateSceneFocusInfo(width, height, samples);
	CreateSceneCloak(width, height, samples);
	...
	VkImageTransition()
		...
		.AddImage(&SceneFocusInfo, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, true)
		.AddImage(&SceneCloak, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, true)
...
void VkRenderBuffers::CreateSceneFocusInfo(int width, int height, VkSampleCountFlagBits samples)
{
	ImageBuilder builder;
	builder.Size(width, height);
	builder.Samples(samples);
	builder.Format(SceneFocusInfoFormat);
	builder.Usage(VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT);
	if (!builder.IsFormatSupported(fb->device.get(), VK_FORMAT_FEATURE_COLOR_ATTACHMENT_BIT | VK_FORMAT_FEATURE_SAMPLED_IMAGE_BIT))
	{
		SceneFocusInfoFormat = VK_FORMAT_R8G8B8A8_UNORM;
		builder.Format(SceneFocusInfoFormat);
	}
	builder.DebugName("VkRenderBuffers.SceneFocusInfo");

	SceneFocusInfo.Image = builder.Create(fb->device.get());

	SceneFocusInfo.View = ImageViewBuilder()
		.Image(SceneFocusInfo.Image.get(), SceneFocusInfoFormat)
		.DebugName("VkRenderBuffers.SceneFocusInfoView")
		.Create(fb->device.get());
}

void VkRenderBuffers::CreateSceneCloak(int width, int height, VkSampleCountFlagBits samples)
{
	ImageBuilder builder;
	builder.Size(width, height);
	builder.Samples(samples);
	builder.Format(SceneCloakFormat);
	builder.Usage(VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT);
	if (!builder.IsFormatSupported(fb->device.get(), VK_FORMAT_FEATURE_COLOR_ATTACHMENT_BIT | VK_FORMAT_FEATURE_SAMPLED_IMAGE_BIT))
	{
		SceneCloakFormat = VK_FORMAT_R8G8B8A8_UNORM;
		builder.Format(SceneCloakFormat);
	}
	builder.DebugName("VkRenderBuffers.SceneCloak");

	SceneCloak.Image = builder.Create(fb->device.get());
	SceneCloak.View = ImageViewBuilder()
		.Image(SceneCloak.Image.get(), SceneCloakFormat)
		.DebugName("VkRenderBuffers.SceneCloakView")
		.Create(fb->device.get());
}
