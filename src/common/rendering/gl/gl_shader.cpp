...
bool FShader::Load(const char * name, const char * vert_prog_lump, const char * frag_prog_lump, const char * proc_prog_lump, const char * light_fragprog, const char * defines)
{
	...
	FString i_data = R"(
		...
		uniform float uFocusHighlight;
		uniform vec4 uFocusTintColor;
		uniform float uFocusTintAmount;
		uniform int uFocusTintStyle;
		uniform vec4 uFocusOutlineColor;
		uniform float uFocusOutlineThickness;
		uniform vec4 uNkVisThruWallTint;
		uniform float uNkVisThruWallSpriteAlpha;
		uniform float uNkVisThruWallOutlineThickness;
		uniform float uNkVisThruWallOutlineAlpha;
		uniform float uNkCloakAmount;
		...
		// [Nakara] Receiver-shader sprite shadow projectors.
		// This buffer must not depend on SHADER_STORAGE_LIGHTS. Some OpenGL paths use
		// UBO dynamic lights while still compiling as #version 430 with SUPPORTS_SHADOWMAPS,
		// so putting this block behind SHADER_STORAGE_LIGHTS makes nkProjectors upload
		// successfully but the material shader never samples them. Binding 8 is OpenGL-only
		// for now; Vulkan descriptors are intentionally not wired yet.
		#ifdef SUPPORTS_SHADOWMAPS
		#define NK_RECEIVER_SPRITE_SHADOWS_SUPPORTED
		struct NkSpriteProjector
		{
			vec4 lightRadius;
			vec4 originStrength;
			vec4 rightSoftness;
		...
		layout(std430, binding = 8) buffer NkSpriteProjectorBuffer
		{
			NkSpriteProjector nkSpriteProjectors[];
		};
		#endif
...
//
	...
	muFocusHighlight.Init(hShader, "uFocusHighlight");
	muFocusTintColor.Init(hShader, "uFocusTintColor");
	muFocusTintAmount.Init(hShader, "uFocusTintAmount");
	muFocusTintStyle.Init(hShader, "uFocusTintStyle");
	muFocusOutlineColor.Init(hShader, "uFocusOutlineColor");
	muFocusOutlineThickness.Init(hShader, "uFocusOutlineThickness");
	muNkVisThruWallTint.Init(hShader, "uNkVisThruWallTint");
	muNkVisThruWallSpriteAlpha.Init(hShader, "uNkVisThruWallSpriteAlpha");
	muNkVisThruWallOutlineThickness.Init(hShader, "uNkVisThruWallOutlineThickness");
	muNkVisThruWallOutlineAlpha.Init(hShader, "uNkVisThruWallOutlineAlpha");
	muNkCloakAmount.Init(hShader, "uNkCloakAmount");
