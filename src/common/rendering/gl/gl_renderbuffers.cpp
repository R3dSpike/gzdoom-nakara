...
void FGLRenderBuffers::ClearScene()
{
	...
	if (mSceneUsesTextures)
	{
		...
		DeleteTexture(mSceneFocusInfoTex);
		DeleteTexture(mSceneCloakTex);
	...
	else
	{
		...
		DeleteRenderBuffer(mSceneFocusInfoBuf);
		DeleteRenderBuffer(mSceneCloakBuf);
...
void FGLRenderBuffers::Setup(int width, int height, int sceneWidth, int sceneHeight)
{
	...
	bool needsSceneTextures = (gl_ssao != 0) || nk_focus_mask_enable || nk_cloak_enable;

	// Nakara focus and cloak auxiliary textures are sampled by regular postprocess
	// shaders. Until these buffers have a proper MSAA resolve path, force the
	// scene to single-sample whenever either feature is active. This prevents
	// GL_TEXTURE_2D_MULTISAMPLE attachments from being bound to sampler2D inputs.
	if (nk_focus_mask_enable || nk_cloak_enable)
	{
		samples = 0;
	}
...
void FGLRenderBuffers::CreateScene(int width, int height, int samples, bool needsSceneTextures)
{
	...
	if (samples > 1)
	{
		if (needsSceneTextures)
		{
			...
			mSceneFocusInfoTex = Create2DMultisampleTexture("SceneFocusInfo", GL_RGBA8, width, height, samples, false);
			mSceneCloakTex = Create2DMultisampleTexture("SceneCloak", GL_R8, width, height, samples, false);
			mSceneFB = CreateFrameBuffer("SceneFB", mSceneMultisampleTex, {}, {}, {}, {}, mSceneDepthStencilTex, true);
			mSceneDataFB = CreateFrameBuffer("SceneGBufferFB", mSceneMultisampleTex, mSceneFogTex, mSceneNormalTex, mSceneFocusInfoTex, mSceneCloakTex, mSceneDepthStencilTex, true);
		}
		else
		{
	...
	else
	{
		if (needsSceneTextures)
		{
			...
			mSceneFocusInfoTex = Create2DTexture("SceneFocusInfo", GL_RGBA8, width, height);
			mSceneCloakTex = Create2DTexture("SceneCloak", GL_R8, width, height);
			mSceneFB = CreateFrameBuffer("SceneFB", mPipelineTexture[0], {}, {}, {}, {}, mSceneDepthStencilTex, false);
			mSceneDataFB = CreateFrameBuffer("SceneGBufferFB", mPipelineTexture[0], mSceneFogTex, mSceneNormalTex, mSceneFocusInfoTex, mSceneCloakTex, mSceneDepthStencilTex, false);
		}
		else
		{
...
PPGLFrameBuffer FGLRenderBuffers::CreateFrameBuffer(const char *name, PPGLTexture colorbuffer0, PPGLTexture colorbuffer1, PPGLTexture colorbuffer2, PPGLTexture colorbuffer3, PPGLTexture colorbuffer4, PPGLTexture depthstencil, bool multisample)
{
	PPGLFrameBuffer fb;
	glGenFramebuffers(1, &fb.handle);
	glBindFramebuffer(GL_FRAMEBUFFER, fb.handle);
	...
	if (multisample)
	{
		...
		if (colorbuffer3.handle != 0)
			glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT3, GL_TEXTURE_2D_MULTISAMPLE, colorbuffer3.handle, 0);
		if (colorbuffer4.handle != 0)
			glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT4, GL_TEXTURE_2D_MULTISAMPLE, colorbuffer4.handle, 0);
	...
	else
	{
		...
		if (colorbuffer3.handle != 0)
			glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT3, GL_TEXTURE_2D, colorbuffer3.handle, 0);
		if (colorbuffer4.handle != 0)
			glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT4, GL_TEXTURE_2D, colorbuffer4.handle, 0);
...
//==========================================================================
//
// Binds the scene focus info texture to the specified texture unit
//
//==========================================================================

void FGLRenderBuffers::BindSceneFocusInfoTexture(int index)
{
	glActiveTexture(GL_TEXTURE0 + index);
	if (mSamples > 1)
		glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, mSceneFocusInfoTex.handle);
	else
		glBindTexture(GL_TEXTURE_2D, mSceneFocusInfoTex.handle);
}
...
void FGLRenderBuffers::BindSceneCloakTexture(int index)
{
	glActiveTexture(GL_TEXTURE0 + index);
	if (mSamples > 1)
		glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, mSceneCloakTex.handle);
	else
		glBindTexture(GL_TEXTURE_2D, mSceneCloakTex.handle);
}
...
void GLPPRenderState::Draw()
{
	...
	for (unsigned int index = 0; index < Textures.Size(); index++)
	{
		...
		case PPTextureType::SceneFocusInfo:
			buffers->BindSceneFocusInfoTexture(index);
			break;

		case PPTextureType::SceneCloak:
			buffers->BindSceneCloakTexture(index);
			break;
