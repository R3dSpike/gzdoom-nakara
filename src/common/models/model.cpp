...
// [Nakara] Reduce arbitrary model geometry to a small convex support set.
// The 26 {-1,0,1} directions preserve the outer envelope while keeping
// runtime silhouette projection independent of the model's original vertex count.
void FModel::SetParticleTrailHull(const TArray<FVector3>& points)
{
	mParticleTrailHull.Clear();
	...
	for (int dz = -1; dz <= 1; ++dz)
	{
		for (int dy = -1; dy <= 1; ++dy)
		{
			for (int dx = -1; dx <= 1; ++dx)
			{
				...
				FVector3 dir((float)dx, (float)dy, (float)dz);
				dir.MakeUnit();
				float bestDot = -FLT_MAX;
				unsigned bestIndex = 0;
				for (unsigned i = 0; i < points.Size(); ++i)
				{
					float d = points[i] | dir;
					if (d > bestDot)
					{
						bestDot = d;
						bestIndex = i;
				...
				const FVector3 candidate = points[bestIndex];
				bool duplicate = false;
				for (unsigned i = 0; i < mParticleTrailHull.Size(); ++i)
				{
					if ((mParticleTrailHull[i] - candidate).LengthSquared() < 0.000001f)
					{
						duplicate = true;
						break;
					}
				}
				if (!duplicate) mParticleTrailHull.Push(candidate);
			}
		}
	}
}
