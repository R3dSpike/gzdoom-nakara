...
public:
	...
	// [Nakara] Compact local-space convex support set used by the connected
	// model silhouette trail. Model loaders feed their CPU geometry into this
	// once; runtime trail rendering then projects only a few dozen points.
	void SetParticleTrailHull(const TArray<FVector3>& points);
	const TArray<FVector3>& GetParticleTrailHull() const { return mParticleTrailHull; }
...
private:
	...
	TArray<FVector3> mParticleTrailHull;
};
