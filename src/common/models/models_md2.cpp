...
void FDMDModel::BuildVertexBuffer(FModelRenderer *renderer)
{
	if (!GetVertexBuffer(renderer->GetType()))
	{
		...
		if (GetParticleTrailHull().Size() == 0 && framevtx != nullptr)
		{
			TArray<FVector3> trailPoints;
			trailPoints.Reserve(info.numFrames * info.numVertices);
			for (int f = 0; f < info.numFrames; ++f)
			{
				for (int v = 0; v < info.numVertices; ++v)
				{
					const auto &p = framevtx[f].vertices[v];
					trailPoints.Push(FVector3(p.xyz[0], p.xyz[1], p.xyz[2]));
				}
			}
			SetParticleTrailHull(trailPoints);
		}
