...
void FMD3Model::BuildVertexBuffer(FModelRenderer *renderer)
{
	if (!GetVertexBuffer(renderer->GetType()))
	{
		...
		if (GetParticleTrailHull().Size() == 0)
		{
			TArray<FVector3> trailPoints;
			for (unsigned s = 0; s < Surfaces.Size(); ++s)
			{
				const MD3Surface &surf = Surfaces[s];
				for (unsigned v = 0; v < Frames.Size() * surf.numVertices; ++v)
				{
					const MD3Vertex &p = surf.Vertices[v];
					trailPoints.Push(FVector3(p.x, p.z, p.y));
				}
			}
