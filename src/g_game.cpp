...
CVAR (Float,	nk_zoom_interp_speed, 0.10f, CVAR_GLOBALCONFIG|CVAR_ARCHIVE)	// Near-target interpolation fraction per tic.
CVAR (Float,	nk_zoom_interp_far_multiplier, 1.00f, CVAR_GLOBALCONFIG|CVAR_ARCHIVE)	// Multiplier applied to the interpolation fraction for large FOV differences.
CVAR (Float,	nk_zoom_interp_range, 45.0f, CVAR_GLOBALCONFIG|CVAR_ARCHIVE)	// FOV difference in degrees at which the far-speed multiplier is fully applied.
CVAR (Float,	nk_zoom_interp_snap, 0.10f, CVAR_GLOBALCONFIG|CVAR_ARCHIVE)	// Snap to the target inside this many degrees to avoid a long interpolation tail.
...
//
extern void NKWarmup_AutoTicker(void);
...
void G_Ticker ()
{
	...
	// [NKS] Actor-targeted texture warmup queue.
	// This is main-thread only and runs in small batches so it can be used in hub/sanctuary stages.
	NKWarmup_AutoTicker();
