...
#ifdef _WIN32
	// [NKS] Only consume the legacy gzdoom ini after Nakara's real config
	// was saved successfully. Explicit writeini <file> exports do not count.
	if (success && filename == nullptr && GameConfig->IsOkayToWriteConfig()
		&& M_IsMigratingLegacyGZDoomPortableConfig())
	{
		M_FinalizeLegacyGZDoomPortableConfigMigration();
	}
#endif
