...
// Keep the existing Actor DistanceCheck property behavior while avoiding
// software-renderer sprite setup work for actors outside their render distance.
...
namespace swrenderer
{
	...
	void RenderOpaquePass::AddSprites(sector_t *sec, int lightlevel, WaterFakeSide fakeside, bool foggy, FDynamicColormap *basecolormap)
	{
		...
		for (auto p = sec->touching_renderthings; p != nullptr; p = p->m_snext)
		{
			...
			if (NK_ShouldSkipDistanceCheckedThing(thing, Thread->Viewport->viewpoint.Pos))
			{
				continue;
			}
