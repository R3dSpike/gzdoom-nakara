...
// Nakara: DistanceCheck early reject helper for decorative actors.
// This keeps the existing Actor DistanceCheck property behavior, but avoids
// building sprite/model render items when the actor is already outside its
// assigned render distance.
static inline bool NK_ShouldSkipDistanceCheckedThing(AActor *thing, const DVector3 &viewpos)
{
	...
	double dx = thing->X() - viewpos.X;
	double dy = thing->Y() - viewpos.Y;
	double dz = thing->Z() - viewpos.Z;
	double checkdist = double(check);
	return dx * dx + dy * dy + dz * dz >= checkdist * checkdist;
}

static inline bool NK_IsWithinSpriteShadowDistance(AActor *thing, const DVector3 &viewpos)
{
	double dx = thing->X() - viewpos.X;
	double dy = thing->Y() - viewpos.Y;
	double dz = thing->Z() - viewpos.Z;
	double checkdist = r_actorspriteshadowdist;
	return dx * dx + dy * dy + dz * dz <= checkdist * checkdist;
}
...
CUSTOM_CVAR(Float, nk_line_distance_cull, 0.f, CVAR_ARCHIVE | CVAR_GLOBALCONFIG)
{
}

static double PointToSegmentDistanceSquared(const FVector2 &p, const FVector2 &a, const FVector2 &b)
{
	FVector2 ab = b - a;
	FVector2 ap = p - a;
...
inline bool IsDistanceCulled(seg_t *seg)
{
	double maxdist = nk_line_distance_cull;
	...
	// First pass: keep this very conservative.
	// Portals, horizon lines, polyobjects, 3D mid textures and transparent/dithered mids
	// are too easy to break if they are used as distance-cull occluders.
	...
	if (seg->sidedef->Flags & WALLF_DITHERTRANS_MID)
	{
		return false;
	}

	if (seg->linedef->flags & ML_3DMIDTEX)
	{
		return false;
	}

	// Very conservative first version: only one-sided solid walls.
	// Two-sided lines can be enabled later after portal/sky/3D floor testing.
	if (seg->backsector)
	{
		return false;
	}

	FVector2 viewpos(r_viewpoint.Pos.X, r_viewpoint.Pos.Y);
	FVector2 p1(seg->v1->fX(), seg->v1->fY());
	FVector2 p2(seg->v2->fX(), seg->v2->fY());

	// Avoid the classic long-line problem: a long seg can pass close to the camera
	// while both endpoints are far away. In that case it must not be culled.
	if (PointToSegmentDistanceSquared(viewpos, p1, p2) <= maxdist2)
	{
		return false;
	}

	double dist1 = (seg->v1->fPos() - r_viewpoint.Pos).LengthSquared();
	double dist2 = (seg->v2->fPos() - r_viewpoint.Pos).LengthSquared();

	return (dist1 > maxdist2) && (dist2 > maxdist2);
}
...
void HWDrawInfo::AddLine (seg_t *seg, bool portalclip)
{
	...
	// Do not distance-cull while drawing through portals.
	// Even if the portal line itself is excluded, lines rendered in the portal pass
	// can still be clipped/LOD-processed incorrectly and cause bad portal textures.
	if (!portalclip && mCurrentPortal == nullptr && IsDistanceCulled(seg))
	{
		isculled = true;
...
void HWDrawInfo::RenderThings(subsector_t * sub, sector_t * sector)
{
	...
	for (auto p = sec->touching_renderthings; p != nullptr; p = p->m_snext)
	{
		...
		// Do this before DistanceCheck so fully rejected map sections do not pay any distance math.
		if (!CurrentMapSections[thing->subsector->mapsection]) continue;

		if (NK_ShouldSkipDistanceCheckedThing(thing, vp.Pos)) continue;

		HWSprite sprite;

		// [Nash] draw sprite shadow
		if (R_ShouldDrawSpriteShadow(thing) && NK_IsWithinSpriteShadowDistance(thing, vp.Pos))
		{
