...
public:
	...
	bool fullbright;
	bool polyoffset;
	bool isSpriteShadow;	// [Nakara] True for generated sprite-shadow copies; suppress focus/VisThru outline state.
	bool isParticleTrailMesh = false; // [Nakara] Connected 2D/3D silhouette trail geometry.
	int particleTrailVertexCount = 0;
	uint8_t particleTrailSource = 0; // 1=sprite silhouette, 2=model silhouette.
	uint32_t particleTrailGeneration = 0; // [Nakara] Exact ribbon generation rendered by this draw copy.
	uint8_t particleTrailSeamCap = 0; // [Nakara] 0=main strip, 1=portal-entry feather cap, 2=portal-exit feather cap.
	uint8_t particleTrailTailFadePass = 0; // [Nakara] 0=body, 1..16=end-feather alpha buckets.
	float particleTrailWidthScale = 1.0f; // [Nakara] Per-draw ribbon width multiplier; used by the optional glow layer.
	int particleTrailRenderPortalGroup = 0; // [Nakara] Portal-group coordinate space used by this draw copy.
	...
	AActor * actor;
	particle_t * particle;
	class DVisualThinker *particleTrailVisual = nullptr; // [Nakara] Detached ribbon owner, mutually exclusive with actor.
	TArray<lightlist_t> *lightlist;
	DRotator Angles;
...
public:
	...
	void CreateParticleTrailVertices(HWDrawInfo *di);
