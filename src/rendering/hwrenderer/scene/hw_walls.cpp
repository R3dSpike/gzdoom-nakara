...
CUSTOM_CVAR(Float, nk_wall_shader_lod_distance, 0.f, CVAR_ARCHIVE | CVAR_GLOBALCONFIG)
{
}

static bool nk_ShouldDisableWallHardwareShader(FGameTexture* texture, const HWSeg& glseg)
{
	if (nk_wall_shader_lod_distance <= 0.f || texture == nullptr)
	{
		return false;
	}

	// Only disable user hardware shaders. Built-in material shaders such as warp/palette/etc.
	// are left alone unless the texture actually has a custom hardware shader assigned.
	if (texture->GetShaderIndex() < FIRST_USER_SHADER)
	{
		return false;
	}

	double dx1 = glseg.x1 - r_viewpoint.Pos.X;
	double dy1 = glseg.y1 - r_viewpoint.Pos.Y;
	double dx2 = glseg.x2 - r_viewpoint.Pos.X;
	double dy2 = glseg.y2 - r_viewpoint.Pos.Y;
	double dist1 = dx1 * dx1 + dy1 * dy1;
	double dist2 = dx2 * dx2 + dy2 * dy2;
	double maxdist = nk_wall_shader_lod_distance;
...
void HWWall::RenderTexturedWall(HWWallDispatcher*di, FRenderState &state, int rflags)
{
	...
	int shaderOverride = nk_ShouldDisableWallHardwareShader(texture, glseg) ? SHADER_Default : -1;
	state.SetMaterial(texture, UF_Texture, 0, flags & 3, NO_TRANSLATION, shaderOverride);
#ifdef NPOT_EMULATION
		...
		else
		{
			...
			if (texture && texture->isValid())
			{
				DoTexture(di, RENDERWALL_TOP, seg, true,
					crefz, frefz, fch1, fch2, ffh1, ffh2, 0, 0);
			}
		}
		return;
	}
