...
{
	// [Nakara] During the through-wall candidate pass, do not add sprites to
	// the normal draw lists. This keeps the feature separate from the normal
	// renderer traversal and from the existing FocusHighlight path.
	if (VisThruWallCollectPass)
	{
		auto newsprt = VisThruWallDrawList.NewSprite();
		*newsprt = *sprite;
		return;
	}

	// [Nakara] +NOVISTHRU actors are collected into a small stencil occluder list.
	// They are drawn colorless just before the through-wall redraw so specific
	// sprite-shaped UI/world objects can block the X-ray silhouette.
	if (VisThruWallOccluderCollectPass)
	{
		auto newsprt = VisThruWallOccluderDrawList.NewSprite();
		*newsprt = *sprite;
		return;
	}

	// [Nakara] Ordinary sprite pixels can also hide a +VISTHRUWALL actor.
	// This depth-only mask pass is separate from +NOVISTHRU: normal sprites reveal
	// the through-wall silhouette, while +NOVISTHRU sprites still block it.
	if (VisThruWallSpriteMaskCollectPass)
	{
		auto newsprt = VisThruWallSpriteMaskDrawList.NewSprite();
		*newsprt = *sprite;

		// Force the depth mask copy down the alpha-tested sprite path. Some
		// ordinary sprites are classified as solid for their normal draw, which is
		// fine visually but can turn the mask into the full billboard rectangle.
		// The mask pass must use the texture alpha so transparent canvas pixels do
		// not write depth. This does not affect the original actor or normal draw.
		newsprt->RenderStyle = STYLE_Translucent;
		newsprt->trans = 1.0f;
		newsprt->hw_styleflags = STYLEHW_Normal;
		newsprt->OverrideShader = -1;
		return;
	}
