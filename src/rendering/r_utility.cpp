...
void R_SetupFrame(FRenderViewpoint& viewPoint, const FViewWindow& viewWindow, AActor* const actor)
{
	...
	if (!paused)
	{
		...
		{
			// Camera motion mode 0 disables quake shake, while modes 1 and 2 retain it.
			// Preserve r_quakeintensity itself so changing the mode restores the configured strength.
			const double quakeFactor = cl_movebob_enabled >= 1 ? r_quakeintensity : 0.0;
