...
// [Nakara] Category bits for the +VISTHRUWALL filter.
// VIS_NONE means this actor is currently not revealed through walls.
// VIS_ALL enables every currently defined category in the global filter.
enum ENakaraVisThruWallMask
{
	VIS_NONE	= 0,
	VIS_MONST	= 1 << 0,
	VIS_ITEM	= 1 << 1,
...
enum ActorFlag9
{
	...
	MF9_FOCUSHIGHLIGHT			= 0x00000200,	// [Nakara] Keep this actor in full color during focus highlight postprocess.
	MF9_FOCUSTINTOUTLINE		= 0x00000400,	// [Nakara] Enable per-actor focus outline data.
	MF9_VISTHRUWALL			= 0x00000800,	// [Nakara] Keep this actor eligible for the separate through-wall redraw pass.
	MF9_NOVISTHRU			= 0x00001000,	// [Nakara] This actor occludes the separate through-wall redraw pass.
	MF9_VISNOTHRUMASK		= 0x00002000,	// [Nakara] Exclude this actor from VisThruWall sprite mask collection.
	MF9_NKSTATICENVIRONMENT	= 0x00004000,	// [Nakara] Skip CallTick() for harmless static environment decorations.
	MF9_NKSHADOWCAST		= 0x00008000,	// [Nakara] Actor is allowed to cast projected sprite shadows.
	MF9_CLOAK				= 0x00010000,	// [Nakara] StarCraft-style refractive cloak. Actor Alpha controls visibility.
};
...
enum ActorRenderFlag2
{
	...
	RF2_NOFOGONSPRITE			= 0x0800,	// [Nakara] Actor sprite is not affected by distance/sector fog.
};
...
// [Nakara] +PARTICLETRAIL source selection. Auto follows the actor's actual
// hardware render source, while Sprite/Model force one path explicitly.
enum EParticleTrailSourceMode : uint8_t
{
	PTTRL_Auto = 0,
	PTTRL_Sprite,
	PTTRL_Model,
...
// [Nakara] 2D sprite trail construction mode. Model trails always use the
// center-line ribbon path regardless of this setting.
enum EParticleTrailSpriteMode : uint8_t
{
	PTTRL2D_AfterImage = 0,
	PTTRL2D_Ribbon,
};

// [Nakara] +PARTICLETRAIL color application mode.
// PTTRL_Stencil uses only the source sprite alpha/silhouette and replaces its RGB
// with ParticleTrail.Color. PTTRL_ColorTint preserves the sprite RGB and tints it.
enum EParticleTrailColorMode : uint8_t
{
	PTTRL_Stencil = 0,
	PTTRL_ColorTint,
};
...
// NOTE: The first member variable *must* be snext.
	...
	PalEntry			ParticleTrailColor;		// [Nakara] Color used by +PARTICLETRAIL.
	bool				bParticleTrailColorSet;	// [Nakara] Allows black to be distinguished from an unset color.
	EParticleTrailSourceMode ParticleTrailSourceMode; // [Nakara] Auto/Sprite/Model source selection.
	EParticleTrailSpriteMode ParticleTrailSpriteMode; // [Nakara] AfterImage or center-line ribbon.
	EParticleTrailColorMode ParticleTrailColorMode; // [Nakara] Stencil (solid color) or source-RGB tint mode.
	double			ParticleTrailLifetime;	// [Nakara] Trail lifetime in seconds. 0 uses the engine default.
	double			ParticleTrailAlpha;		// [Nakara] Trail alpha. 0 uses the engine default.
	double			ParticleTrailTailAlphaFade; // [Nakara] 0..1 spatial fraction used by TailFeather. TailAlphaFade remains a compatibility alias.
	double			ParticleTrailHeadFeather; // [Nakara] 0..1 spatial fraction used by HeadFeather. 0 disables it.
	double			ParticleTrailWaveAmplitude; // [Nakara] World-unit lateral center-line displacement. 0 disables wave.
	double			ParticleTrailWaveFrequency; // [Nakara] Full wave cycles per 64 world units of trail.
	double			ParticleTrailWaveSpeed; // [Nakara] Wave phase cycles per second; positive values travel toward the tail.
	double			ParticleTrailGlowScale;	// [Nakara] Optional outer glow width multiplier relative to the core ribbon. 0 disables glow.
	double			ParticleTrailGlowAlpha;	// [Nakara] Optional outer glow alpha.
	PalEntry			ParticleTrailGlowColor;	// [Nakara] Optional outer glow color; inherits ParticleTrail.Color when unset.
	bool				bParticleTrailGlowColorSet;
	double			ParticleTrailScale;		// [Nakara] Extra trail scale. 0 uses the engine default.
	double			ParticleTrailDensity;	// [Nakara] Afterimage density / ribbon path sampling density.
	TArray<FParticleTrailHistorySample> ParticleTrailHistory; // [Nakara] Runtime center-line samples for ribbon geometry.
	uint32_t		ParticleTrailGeneration = 0; // [Nakara] Unique ribbon chain for this actor instance/activation.
	uint8_t		ParticleTrailHistoryMode = 0xff; // [Nakara] Clears incompatible history when source/mode changes.
	DVector3		ParticleTrailLaunchPosition; // [Nakara] Exact visual position immediately before this actor's first Tick().
	int			ParticleTrailLaunchPortalGroup = -1; // [Nakara] Portal space for the launch anchor.
	int			ParticleTrailLaunchMapTime = -1; // [Nakara] Only valid during the actor's spawn tic.
	bool			bParticleTrailLaunchAnchorValid = false; // [Nakara] Runtime-only exact first-tic ribbon anchor.
	FRenderStyle	RenderStyle;		// Style to draw this actor with
	FTextureID		picnum;				// Draw this instead of sprite if valid
	uint32_t			fillcolor;			// Color to draw when STYLE_Shaded
	PalEntry			FocusTintColor;		// [Nakara] Extra tint color used only by the focus highlight path.
	double			FocusTintAmount;		// [Nakara] Extra tint amount used only by the focus highlight path.
	int			FocusTintStyle;		// [Nakara] Focus tint blend style.
	bool			bFocusTintColorSet;		// [Nakara] True if FocusTintColor was explicitly set.
	PalEntry			FocusTintOutlineColor;	// [Nakara] Optional per-actor focus outline color.
	bool			bFocusTintOutlineColorSet;	// [Nakara] True if FocusTintOutlineColor was explicitly set.
	double			FocusTintOutlineThickness;	// [Nakara] Per-actor focus outline thickness in pixels.
	PalEntry			VisThruWallColor;		// [Nakara] Tint color used only by the separate through-wall redraw pass.
	bool			bVisThruWallColorSet;	// [Nakara] True if VisThruWallColor was explicitly set.
	double			VisThruWallAlpha;		// [Nakara] Tint strength used only by the separate through-wall redraw pass.
	bool			bVisThruWallAlphaSet;	// [Nakara] True if VisThruWallAlpha was explicitly set.
	double			VisThruWallSpriteAlpha;	// [Nakara] Final sprite opacity used only by the separate through-wall redraw pass.
	bool			bVisThruWallSpriteAlphaSet;	// [Nakara] True if VisThruWallSpriteAlpha was explicitly set.
	double			VisThruWallOutlineThickness;	// [Nakara] Texture-alpha outline thickness for the through-wall redraw pass.
	double			VisThruWallOutlineAlpha;	// [Nakara] Outline opacity used only by the separate through-wall redraw pass.
	bool			bVisThruWallOutlineAlphaSet;	// [Nakara] True if VisThruWallOutlineAlpha was explicitly set.
	double			VisThruWallMaxDistance;	// [Nakara] Maximum distance for the through-wall redraw pass. 0 means unlimited.
	double			VisThruWallFadeDistance;	// [Nakara] Fade-out range before VisThruWallMaxDistance. 0 disables distance fade.
	double			VisThruWallFadeInTics;	// [Nakara] Time to fade the through-wall effect in. 0 means instant.
	double			VisThruWallFadeOutTics;	// [Nakara] Time to fade the through-wall effect out. 0 means instant.
	uint32_t		VisThruWallMask;	// [Nakara] Category bits used by the through-wall redraw filter.
	double			VisThruWallCurrentFade;	// [Nakara] Runtime fade strength for the through-wall redraw pass. Not exported to ZScript.
	int			VisThruWallLastFadeTic;	// [Nakara] Runtime tic used to keep fade updates stable at uncapped framerates.
...
// interaction info
	...
	// Nakara navigation state. This is transient on purpose: a loaded actor
	// simply rebuilds its route on the next navigation-aware chase call.
	TArray<DVector3>	nkSmartPath;
	unsigned		nkSmartPathIndex = 0;
	DVector3		nkSmartPathTarget;
	DVector3		nkSmartLastProgressPos;
	DVector3		nkSmartTacticalGoal;
	DVector3		nkSmartLastLiveTarget;
	int			nkSmartLastProgressTime = 0;
	int			nkSmartLastChaseCallTime = -1;
	int			nkSmartNextRepath = 0;
	int			nkSmartNextTacticalUpdate = 0;
	int			nkSmartRoamNextTarget = 0;
	int			nkSmartTacticalSector = 0;
	int			nkSmartRouteSeed = 0;
	int			nkSmartNavigationMode = 0;
	double			nkSmartTacticalSpread = 0.0;
	bool			nkSmartPathFlying = false;
	bool			nkSmartTargetValid = false;
	bool			nkSmartProgressValid = false;
	bool			nkSmartTacticalValid = false;
	bool			nkSmartLiveTargetValid = false;
	bool			nkSmartDisconnectedRoam = false;
