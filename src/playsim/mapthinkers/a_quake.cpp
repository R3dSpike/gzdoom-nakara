...
#ifdef _WIN32
	// Engine rumble for quakes (dynamic, respects SCALEUP/SCALEDOWN via GetModIntensity)
	{
		static int s_lastRumbleTic[MAXPLAYERS] = { 0 };
		const int nowTic = m_Spot->Level->time;
		const int minIntervalTic = 2; // per-player rate limit
		const int updateMask = 3;     // update every 4 tics (~114ms)
		if ( (nowTic & updateMask) == 0 )
		{
			// 40% reduction overall (i.e., keep 60%)
			const double globalScale = 0.60;

			// Fade-out envelope near the end, regardless of SCALE flags
			const int fadeTics = 20; // ~0.57s
			double endFade = 1.0;
			if (m_Countdown > 0 && m_Countdown < fadeTics)
			{
				const double t = clamp<double>((double)m_Countdown / (double)fadeTics, 0.0, 1.0);
				endFade = t * t * (3.0 - 2.0 * t); // smoothstep
			}

			for (int pnum = 0; pnum < MAXPLAYERS; ++pnum)
			{
				if (!m_Spot->Level->PlayerInGame(pnum)) continue;
				if (nowTic - s_lastRumbleTic[pnum] < minIntervalTic) continue;

				auto pl = m_Spot->Level->Players[pnum];
				if (!pl || !pl->mo) continue;

				// Distance check
				const double dist = (m_Flags & QF_3D) ? m_Spot->Distance3D(pl->mo) : m_Spot->Distance2D(pl->mo);
				if (dist >= m_TremorRadius) continue;

				// Intensity (0..1), respects QF_SCALEUP/QF_SCALEDOWN/QF_MAX/QF_FULLINTENSITY via GetModIntensity
				double I = max(max(GetModIntensity(m_Intensity.X), GetModIntensity(m_Intensity.Y)), GetModIntensity(m_Intensity.Z)) / 9.0;

				// Tremor radius falloff
				const double falloff = GetFalloff(dist, m_TremorRadius);
				I *= falloff;

				// Global reduction + end fade
				I *= globalScale * endFade;

				if (I <= 0.0) continue;

				// Map to motors (keep high relatively low to avoid “tingly” fatigue)
				const int tics = 8;
				const int low  = (int)clamp<double>( 6000.0 + I * 42000.0, 0.0, 65535.0);
				const int high = (int)clamp<double>(  800.0 + I *  9000.0, 0.0, 20000.0);

				if (low > 0 || high > 0)
				{
					I_XInputRumbleTics(pnum, low, high, tics);
					s_lastRumbleTic[pnum] = nowTic;
				}
			}
		}
	}
