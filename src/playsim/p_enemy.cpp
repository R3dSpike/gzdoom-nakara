...
void A_DoChase (AActor *actor, bool fastchase, FState *meleestate, FState *missilestate, bool playactive, bool nightmarefast, bool dontmove, int flags, bool hypnotizechase, bool smartchase)
{
	...
	// A_HypnotizeChase follows only the target assigned by ZScript. Unlike the
	// combat-oriented A_Chase, it must not acquire a replacement when that
	// target is temporarily non-shootable (for example, a phasing ghost).
	if (hypnotizechase &&
		(actor->target == nullptr || (actor->target->ObjectFlags & OF_EuthanizeMe)))
	{
	...
	if (!hypnotizechase && actor->target != NULL &&
	...
	if (!hypnotizechase && actor->target && actor->target != actor->goal && (actor->target->health <= 0 || actor->IsFriend(actor->target)))
		actor->target = nullptr;

	// [RH] Friendly monsters will consider chasing whoever hurts a player if they
	// don't already have a target.
	if (!hypnotizechase && actor->flags & MF_FRIENDLY && actor->target == NULL)
	{
		player_t *player;
	...
	if (!hypnotizechase && (!actor->target || !(actor->target->flags & MF_SHOOTABLE)))
	...
	if (actor->flags & MF_JUSTATTACKED)
	{
		...
		{
			if (smartchase) P_NKSmartChaseMove(actor);
			else if (hypnotizechase) P_NKHypnotizeChaseMove(actor);
			else P_NewChaseDir (actor);
		}
 ...
 nomissile:
	...
	if (!hypnotizechase && (multiplayer || actor->TIDtoHate)
		&& !actor->threshold
	...
	if ((!fastchase || !actor->FastChaseStrafeCount) && !dontmove)
	{
		...
		if (smartchase)
		{
			P_NKSmartChaseMove(actor);
		}
		else if (hypnotizechase)
		{
			P_NKHypnotizeChaseMove(actor);
		}
		else if ((--actor->movecount < 0 && !(flags & CHF_NORANDOMTURN)) || (!P_SmartMove(actor) && !(flags & CHF_STOPIFBLOCKED)))
		{
			P_NewChaseDir(actor);
		}
		...
		if (actor->flags2&MF2_CANTLEAVEFLOORPIC && actor->floorpic != oldFloor )
		{
			...
			if (!(flags & CHF_STOPIFBLOCKED))
			{
				if (hypnotizechase || smartchase) P_NKInvalidateSmartPath(actor);
				else P_NewChaseDir(actor);
			}
		}
	}
...
void A_SmartChaseNative(AActor *self, int meleelabel, int missilelabel, int flags)
{
	FName meleename = ENamedName(meleelabel - 0x10000000);
	FName missilename = ENamedName(missilelabel - 0x10000000);
	if (meleename != NAME__a_chase_default || missilename != NAME__a_chase_default)
	{
		...
		A_DoChase(self, !!(flags&CHF_FASTCHASE), melee, missile, !(flags&CHF_NOPLAYACTIVE),
			!!(flags&CHF_NIGHTMAREFAST), !!(flags&CHF_DONTMOVE), flags & 0x3fffffff, false, true);
	}
	else
	{
		A_DoChase(self, false, self->MeleeState, self->MissileState, true,
			gameinfo.nightmarefast, false, 0, false, true);
	}
}

DEFINE_ACTION_FUNCTION_NATIVE(AActor, A_SmartChase, A_SmartChaseNative)
{
	PARAM_SELF_PROLOGUE(AActor);
	PARAM_STATELABEL(meleelabel);
	PARAM_STATELABEL(missilelabel);
	...
	A_SmartChaseNative(self, meleelabel, missilelabel, flags);

	return 0;
}


DEFINE_ACTION_FUNCTION(AActor, A_KitsuneSpiritMove)
{
	PARAM_SELF_PROLOGUE(AActor);
	PARAM_INT(role);
	PARAM_FLOAT(moveSpeed);
	PARAM_BOOL(hypnotizeConverge);
	ACTION_RETURN_BOOL(P_NKKitsuneSpiritMove(self, role, moveSpeed, hypnotizeConverge));
}

void A_HypnotizeChaseNative(AActor *self, int meleelabel, int missilelabel, int flags)
{
	FName meleename = ENamedName(meleelabel - 0x10000000);
	FName missilename = ENamedName(missilelabel - 0x10000000);
	if (meleename != NAME__a_chase_default || missilename != NAME__a_chase_default)
	{
		...
		A_DoChase(self, !!(flags&CHF_FASTCHASE), melee, missile, !(flags&CHF_NOPLAYACTIVE),
			!!(flags&CHF_NIGHTMAREFAST), !!(flags&CHF_DONTMOVE), flags & 0x3fffffff, true);
	}
	else
	{
		A_DoChase(self, false, self->MeleeState, self->MissileState, true,
			gameinfo.nightmarefast, false, 0, true);
	}
}

DEFINE_ACTION_FUNCTION_NATIVE(AActor, A_HypnotizeChase, A_HypnotizeChaseNative)
{
	PARAM_SELF_PROLOGUE(AActor);
	PARAM_STATELABEL(meleelabel);
	PARAM_STATELABEL(missilelabel);
	...
	A_HypnotizeChaseNative(self, meleelabel, missilelabel, flags);
