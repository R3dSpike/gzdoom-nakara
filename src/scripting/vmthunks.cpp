...
// [NKS] Music slot bridge. Slot 0 shares the legacy main music; slots 1+ are additional streams.
DEFINE_ACTION_FUNCTION(DObject, S_PlayMusicSlot)
{
	PARAM_PROLOGUE;
	PARAM_INT(slotId);
	PARAM_STRING(music);
	PARAM_INT(order);
	PARAM_BOOL(looping);
	ACTION_RETURN_BOOL(S_PlayMusicSlot(slotId, music.GetChars(), order, looping));
}

DEFINE_ACTION_FUNCTION(DObject, S_PlayMusicSlotCrossfade)
{
	PARAM_PROLOGUE;
	PARAM_INT(slotId);
	PARAM_STRING(music);
	PARAM_BOOL(looping);
	PARAM_FLOAT(crossfadeTime);
	ACTION_RETURN_BOOL(S_PlayMusicSlotCrossfade(slotId, music.GetChars(), looping, (float)crossfadeTime));
}

DEFINE_ACTION_FUNCTION(DObject, S_ChangeMusicCrossfade)
{
	PARAM_PROLOGUE;
	PARAM_STRING(music);
	PARAM_BOOL(looping);
	PARAM_FLOAT(crossfadeTime);
	ACTION_RETURN_BOOL(S_ChangeMusicCrossfade(music.GetChars(), looping, (float)crossfadeTime));
}

DEFINE_ACTION_FUNCTION(DObject, S_PlayMusicSequence)
{
	PARAM_PROLOGUE;
	PARAM_INT(slotId);
	PARAM_STRING(intro);
	PARAM_STRING(loop);
	PARAM_STRING(outro);
	ACTION_RETURN_BOOL(S_PlayMusicSequence(slotId, intro.GetChars(), loop.GetChars(), outro.GetChars()));
}

DEFINE_ACTION_FUNCTION(DObject, S_EndMusicSequence)
{
	PARAM_PROLOGUE;
	PARAM_INT(slotId);
	ACTION_RETURN_BOOL(S_EndMusicSequence(slotId));
}

DEFINE_ACTION_FUNCTION(DObject, S_EndMusicSequenceImmediate)
{
	PARAM_PROLOGUE;
	PARAM_INT(slotId);
	PARAM_FLOAT(crossfadeTime);
	ACTION_RETURN_BOOL(S_EndMusicSequenceImmediate(slotId, (float)crossfadeTime));
}

DEFINE_ACTION_FUNCTION(DObject, S_IsMusicSequencePlaying)
{
	PARAM_PROLOGUE;
	PARAM_INT(slotId);
	ACTION_RETURN_BOOL(S_IsMusicSequencePlaying(slotId));
}

DEFINE_ACTION_FUNCTION(DObject, S_BeginMusicStemGroup)
{
	PARAM_PROLOGUE;
	PARAM_INT(slotId);
	PARAM_BOOL(looping);
	ACTION_RETURN_BOOL(S_BeginMusicStemGroup(slotId, looping));
}

DEFINE_ACTION_FUNCTION(DObject, S_AddMusicStem)
{
	PARAM_PROLOGUE;
	PARAM_INT(slotId);
	PARAM_INT(stemId);
	PARAM_STRING(music);
	PARAM_FLOAT(initialVolume);
	ACTION_RETURN_BOOL(S_AddMusicStem(slotId, stemId, music.GetChars(), (float)initialVolume));
}

DEFINE_ACTION_FUNCTION(DObject, S_StartMusicStemGroup)
{
	PARAM_PROLOGUE;
	PARAM_INT(slotId);
	ACTION_RETURN_BOOL(S_StartMusicStemGroup(slotId));
}

DEFINE_ACTION_FUNCTION(DObject, S_SetMusicStemVolume)
{
	PARAM_PROLOGUE;
	PARAM_INT(slotId);
	PARAM_INT(stemId);
	PARAM_FLOAT(volume);
	ACTION_RETURN_BOOL(S_SetMusicStemVolume(slotId, stemId, (float)volume));
}

DEFINE_ACTION_FUNCTION(DObject, S_SetMusicStemVolumeFade)
{
	PARAM_PROLOGUE;
	PARAM_INT(slotId);
	PARAM_INT(stemId);
	PARAM_FLOAT(targetVolume);
	PARAM_FLOAT(fadeTime);
	ACTION_RETURN_BOOL(S_SetMusicStemVolumeFade(slotId, stemId, (float)targetVolume, (float)fadeTime));
}

DEFINE_ACTION_FUNCTION(DObject, S_IsMusicStemGroupPlaying)
{
	PARAM_PROLOGUE;
	PARAM_INT(slotId);
	ACTION_RETURN_BOOL(S_IsMusicStemGroupPlaying(slotId));
}

DEFINE_ACTION_FUNCTION(DObject, S_StopMusicSlot)
{
	PARAM_PROLOGUE;
	PARAM_INT(slotId);
...
DEFINE_ACTION_FUNCTION(DObject, S_StopAllMusicSlots)
{
	PARAM_PROLOGUE;
	S_StopAllMusicSlots();
	return 0;
...
DEFINE_ACTION_FUNCTION(DObject, S_SetMusicSlotVolume)
{
	PARAM_PROLOGUE;
	PARAM_INT(slotId);
	PARAM_FLOAT(volume);
	S_SetMusicSlotVolume(slotId, (float)volume);
	return 0;
}

DEFINE_ACTION_FUNCTION(DObject, S_SetMusicSlotVolumeFade)
{
	PARAM_PROLOGUE;
	PARAM_INT(slotId);
	PARAM_FLOAT(targetVolume);
	PARAM_FLOAT(fadeTime);
	ACTION_RETURN_BOOL(S_SetMusicSlotVolumeFade(slotId, (float)targetVolume, (float)fadeTime));
}

DEFINE_ACTION_FUNCTION(DObject, S_IsMusicSlotPlaying)
{
	PARAM_PROLOGUE;
	PARAM_INT(slotId);
	ACTION_RETURN_BOOL(S_IsMusicSlotPlaying(slotId));
}

DEFINE_ACTION_FUNCTION(DObject, S_PauseMusicSlot)
{
	PARAM_PROLOGUE;
	PARAM_INT(slotId);
	PARAM_BOOL(paused);
	S_PauseMusicSlot(slotId, paused);
	return 0;
}
