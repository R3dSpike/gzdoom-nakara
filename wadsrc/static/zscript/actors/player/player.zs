...
class PlayerPawn : Actor
{
	...
	double LastWeaponSwayAngle;
	double WeaponSwayTurn;
	double WeaponSwaySide;
	double WeaponSwayForward;
	double PrevWeaponSwayTurn;
	double PrevWeaponSwaySide;
	double PrevWeaponSwayForward;
	...
	override void Tick()
	{
		...
		Super.Tick();
		UpdateWeaponBobState();
	}
void UpdateWeaponBobState()
{
    double maxForward;
    double maxSide;
    double forwardInput;
    ...
    if (!player || player.mo != self)
    {
        ...
        PrevWeaponSwayTurn = WeaponSwayTurn;
        PrevWeaponSwaySide = WeaponSwaySide;
        PrevWeaponSwayForward = WeaponSwayForward;
    ...
    if (player.playerstate == PST_DEAD || player.ReadyWeapon == null || player.ReadyWeapon.bDontBob)
    {
        curBob = 0;
    ...
    maxForward = MAX(1.0, gameinfo.normforwardmove[0] * 256.0);
    maxSide = MAX(1.0, gameinfo.normsidemove[0] * 256.0);

    forwardInput = player.cmd.forwardmove / maxForward;
    sideInput = player.cmd.sidemove / maxSide;

    inputLen = sqrt(forwardInput * forwardInput + sideInput * sideInput);
    targetBob = clamp(inputLen, 0.0, 1.0) * MAXBOB;
    ...
    if (abs(targetBob - curBob) <= 1.0)
    {
        curBob = targetBob;
    }
    else
    {
        zoom = MAX(1.0, abs(curBob - targetBob) / 40.0);

        if (curBob > targetBob)
        {
            curBob -= zoom;
            if (curBob < targetBob) curBob = targetBob;
        }
        else
        {
            curBob += zoom;
    ...
    double angleDelta = deltaangle(LastWeaponSwayAngle, Angle);
    LastWeaponSwayAngle = Angle;

    angleDelta = clamp(angleDelta, -8.0, 8.0);

    double targetTurn = angleDelta / 8.0;

    WeaponSwayTurn += (targetTurn - WeaponSwayTurn) * 0.18;
    WeaponSwaySide += (sideInput - WeaponSwaySide) * 0.12;
    WeaponSwayForward += (forwardInput - WeaponSwayForward) * 0.12;
}
	//===========================================================================
	//
	// 
...
if ((DeathTimer<1)&&(
	...
	virtual void CheckFOV()
	{
		...
		{
			double delta = desired - player.FOV;
			double distance = abs(delta);
			double snapDistance = clamp(GetCVar("nk_zoom_interp_snap"), 0.001, 5.0);
			...
			{
				// Use a smooth distance-based response curve. Large FOV changes move
				// quickly, while the interpolation progressively slows near the target.
				double nearSpeed = clamp(GetCVar("nk_zoom_interp_speed"), 0.01, 1.0);
				double farMultiplier = clamp(GetCVar("nk_zoom_interp_far_multiplier"), 0.01, 5.0);
				double responseRange = clamp(GetCVar("nk_zoom_interp_range"), 1.0, 180.0);
				double response = clamp(distance / responseRange, 0.0, 1.0);

				// Smoothstep prevents a visible speed discontinuity as the distance changes.
				response = response * response * (3.0 - 2.0 * response);

				double zoomSpeed = nearSpeed * (1.0 + (farMultiplier - 1.0) * response);
				zoomSpeed = clamp(zoomSpeed, 0.001, 1.0);
				player.FOV += delta * zoomSpeed;
			}
		}
	}
...
virtual Vector2 BobWeapon(double ticfrac)
{
    ...
    bool useSpearSway = weapon is "SharkinSpear";
    bool useWalkBob = player.onground || !useSpearSway;
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        double BobIntensity = (player.WeaponState & WF_WEAPONBOBBING) ? 1.0 : player.GetWBobFire();

        if (curbob != 0 && useWalkBob)
        {
            double bobx = curbob * BobIntensity * Rangex * ViewBob;
            double boby = curbob * BobIntensity * Rangey * ViewBob;
    ...
    result = p1 * (1.0 - ticfrac) + p2 * ticfrac;

    if (useSpearSway)
    {
        double swayTurn = PrevWeaponSwayTurn * (1.0 - ticfrac) + WeaponSwayTurn * ticfrac;
        double swaySide = PrevWeaponSwaySide * (1.0 - ticfrac) + WeaponSwaySide * ticfrac;
        double swayForward = PrevWeaponSwayForward * (1.0 - ticfrac) + WeaponSwayForward * ticfrac;

        double forwardAmount = MAX(0.0, swayForward);

        result.Y += forwardAmount * 5.2;

            double t = Level.maptime + ticfrac;
            Vector2 sway;

            sway.X = sin(t * 3.0) * 0.6
                   + swayTurn * 20.0
                   + swaySide * 10.0;

    double forwardDrop = MAX(0.0, swayForward) * 5.0;

    sway.Y = cos(t * 2.4) * 0.65
           + forwardDrop;
        ...
        if (psp)
        {
            psp.baseScale.x = 1.3 + forwardAmount * 0.10;
            psp.baseScale.y = 1.4 + forwardAmount * 0.12;
        }
    }
