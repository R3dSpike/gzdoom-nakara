...
extend class Object
{
	...
	// [NKS] Actor-targeted warmup bridge.
	native static bool NKWarmupActor(String actorName);
	native static void NKWarmupClear();
	native static void NKWarmupStart();
	native static void NKWarmupStop();
	native static bool NKWarmupLoadingScreen(bool enabled);
	native static int NKWarmupBurst(int count);
	native static int NKWarmupFlushNow();
	native static bool NKWarmupIsDone();
	native static int NKWarmupJobsLeft();
	...
	// [NKS] Music slots. Slot 0 is the main music; slots 1+ are additional streams.
	native static bool S_PlayMusicSlot(int slotId, String music_name, int order = 0, bool looping = false);
	native static bool S_PlayMusicSlotCrossfade(int slotId, String music_name, bool looping = true, double crossfadeTime = 0.5);
	native static bool S_ChangeMusicCrossfade(String music_name, bool looping = true, double crossfadeTime = 0.5);
	native static bool S_PlayMusicSequence(int slotId, String intro_name, String loop_name, String outro_name);
	native static bool S_EndMusicSequence(int slotId);
	native static bool S_EndMusicSequenceImmediate(int slotId, double crossfadeTime = 0.5);
	native static bool S_IsMusicSequencePlaying(int slotId);
	// Sample-synchronized PCM stems. Stem 0 is the master timeline.
	native static bool S_BeginMusicStemGroup(int slotId, bool looping = true);
	native static bool S_AddMusicStem(int slotId, int stemId, String music_name, double initialVolume = 1.0);
	native static bool S_StartMusicStemGroup(int slotId);
	native static bool S_SetMusicStemVolume(int slotId, int stemId, double volume);
	native static bool S_SetMusicStemVolumeFade(int slotId, int stemId, double targetVolume, double fadeTime);
	native static bool S_IsMusicStemGroupPlaying(int slotId);
	native static void S_StopMusicSlot(int slotId);
	native static void S_StopAllMusicSlots();
	native static void S_SetMusicSlotVolume(int slotId, double volume);
	native static bool S_SetMusicSlotVolumeFade(int slotId, double targetVolume, double fadeTime);
	native static bool S_IsMusicSlotPlaying(int slotId);
	native static void S_PauseMusicSlot(int slotId, bool paused = true);
...
struct Shader native
{
	...
	clearscope static void SetFocusTintStrength(PlayerInfo player, float value)
	{
		if (IsConsolePlayer(player)) PPShader.SetFocusTintStrength(value);
	}
}
